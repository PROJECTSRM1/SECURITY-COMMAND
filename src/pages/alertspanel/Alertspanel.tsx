...
//       setSettings(s);
//       // optional: you might want to restart simulator based on s.aiAlerts.simulator
//     }
...
//     // fallback mapping using settings if available
...
//     // person vs vehicle or generic
//     if (typeHint === "person") {
//       const pThresh = ai.person?.suspiciousConfidence ?? 70;
...
//     if (typeHint === "vehicle") {
//       const vThresh = ai.vehicle?.confidenceThreshold ?? 85;
...
//     // small random chance to be person/vehicle to allow threshold testing
//     const r = Math.random();
//     if (r < 0.25) {
//       type = "person";
//       typeHint = "person";
//       // person confidence skewed to show interesting values
...
//     // choose severity from mapping function (so settings reflect in severity)
//     const mappedSeverity = mapSeverityByConfidence(confPct, typeHint) as Severity;

//     // Compose reasoning mentioning thresholds if avail
//     let reasoning = `Model confidence ${confPct}%`;
//     if (typeHint === "person" && settings?.aiAlerts?.person?.suspiciousConfidence) {
//       reasoning += ` (person threshold ${settings.aiAlerts.person.suspiciousConfidence}%)`;
//     }
//     if (typeHint === "vehicle" && settings?.aiAlerts?.vehicle?.confidenceThreshold) {
//       reasoning += ` (vehicle threshold ${settings.aiAlerts.vehicle.confidenceThreshold}%)`;
//     }
...
//   useEffect(() => {
//     // simulator frequency can be controlled by settings
//     const freqSeconds = settings?.aiAlerts?.simulator?.alertsFrequencySeconds ?? 8;
//     const enabled = settings?.aiAlerts?.simulator?.enabled ?? true;

//     if (!enabled) return;

//     const id = setInterval(() => {
//       const a = createMockAlert();
//       if (!a) return; // respect aiAlerts.enabled false
//       setAlerts((s) => [a, ...s].slice(0, 200));
//     }, Math.max(800, freqSeconds * 1000));

//     return () => clearInterval(id);
//     // eslint-disable-next-line react-hooks/exhaustive-deps
//   }, [settings?.aiAlerts?.simulator?.enabled, settings?.aiAlerts?.simulator?.alertsFrequencySeconds, settings?.aiAlerts?.enabled]);
...
//   );
// }
// src/pages/alertspanel/AlertsPanel.tsx
import { useEffect, useMemo, useState } from "react";
import "./AlertsPanel.css";
